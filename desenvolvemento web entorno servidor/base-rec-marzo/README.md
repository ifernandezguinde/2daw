# UD5 - Inicial
Repositorio que puede servir como base para comenzar el propio tema 5. Adaptado a PHP8.0.
## Changelog
* Añadimos restricción UNIQUE a codigo en tabla proveedor.
* Ahora Core/View genera automáticamente la variable `$seccion` con la ruta relativa de la petición sin QueryVars.
* Creada tabla rol y eliminada tabla ~~aux_rol~~. Tabla `rol` es FK de `usuario_sistema`.
* Añadido script que genera tabla `usuario_sistema` con clave `TestTest1`.
* Tabla trabajadores
* Eliminado data-widget de logout
* Adaptaciones a PHP 8.0
  * Añadido type a atributos de clases en Core
  * BaseModel renombrado a BaseDbModel
  * .env Contiene variable de configuración `host.folder`
# Docker: PHP & MySQL
